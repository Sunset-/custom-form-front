<template>
    ...
    <sunset-crud v-ref:assistant :options="options">
    </sunset-crud>
</template>
<script>
    ...
    export default {
        methods: {
            confirmRet(){
                ...
                AssistantStore.retPwd(para).then(res=>{
            ...
            return {
                ...
                options: {
                        ...
                        recordTools: [{
                            ...
                            operate:(v)=>{
                                var clear = Sunset.confirm({
                                    ...
                                    loading: true,
                                    onOk: () => {
                                        return AssistantStore.delete(v.userId).then(res=>{
                                            Sunset.tip("已删除","success");
                                            clear();
                                            this.$refs.assistant.operateSignal("REFRESH");
                                        })
                                    }
                                });
                            }
                    ...
                    formOptions: {
                        ...
                        submit: (data) => {
                            data.userType = Sunset.Service.Dictionary.alias('SYSTEM_ACCOUNT_TYPE', 'ASSISTANT');
                            ...
                            if(JSON.parse(data.entity).id){
                                return AssistantStore.save(data).then(res=>{
                                    Sunset.tip("保存成功","success");
                                });
                            }else{
                                return AssistantStore.isExist({loginName:data.username,agencyCode: GlobalService.getSystemAgencyCode()}).then(res=>{
                                    if(res){
                                        Sunset.tip("账号已存在，请重新输入","warning");
                                        throw new Error();
                                    }else{
                                        Sunset.tip("保存成功","success");
                                        return AssistantStore.save(data);
                                    }
                                })
                            }
                        }
