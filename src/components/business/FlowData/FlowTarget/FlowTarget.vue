    ...
    export default {
        ...
        data() {
            return {
                SEENODE: {},
                snode: {},
                CANT_CHECK_CONFIG: {//禁选节点配置
                    'difficultyflow': [Sunset.Service.Dictionary.alias('ORG_CODE', 'HOSPITAL')]
                },
                SELECT_NODE_CONFIG : {//显示节点配置
                    'ordercheck' : function(c){
                        return c==Sunset.Service.Dictionary.alias('ORG_CODE', 'HOSPITAL');
                    }
                },
                treeOptions: {
                    autoExpand: true,
                        ...
                        res && res.forEach(item => {
                            ...
                            if (item.scenesType & Sunset.Service.Dictionary.alias(
                                ...
                                function addMembers(parent, memberId, parentId, isTree,
                                    ...
                                    var SELECT_NODE_CONFIG = self.SELECT_NODE_CONFIG;
                                    if(!SELECT_NODE_CONFIG[self.type]||SELECT_NODE_CONFIG[self.type](category)){
                                        nodes.push({
