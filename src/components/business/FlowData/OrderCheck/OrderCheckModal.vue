...
<template>
    <Modal :class-name="'transfer-flow-modal'" :visible.sync="visible" title="预约检查申请" :mask-closable="false" :width="1000" @on-cancel="afterCancel">
        <div class="modal-major">
            <div class="flow-target">
                <div style="height:30px;line-height:30px;background:#ccc;padding:0 20px">拟转至</div>
                 <flow-target type="ordercheck" check="single" @checked="checkedTargets"></flow-target> 
            </div>
                    ...
                    fields: [{
                        ...
                        search(word) {
                            return Promise.resolve().then(() => {
                                return $http({
                                    url: '/service/business/relation/hospital/check_item',
                                    type: 'GET',
                                    data: {
                                        hospitalId:'607668aec52347b19dc1fd0c90e10827',
                                        keyword: word
                                    }
                                }).then(res=>{
                                    var checkTypeList=[];
                                    res.forEach(item=>{
                                        checkTypeList.push(`${item.id}-${item.categoryA} / ${item.categoryB} / ${item.item}`)
                                    });
                ...
                toolbarOptions: {
                    tools: [{
                        ...
                        operate: () => {
                            ...
                            if(!model.checkType){
                                Sunset.tip('检查项目不能为空','warning');
                                return;
                            }else{
                                var checkList=[];
                                model.checkType.split(',').forEach(value=>
                                    checkList.push(value.split('-')[0])
                                )
                            }
                            if(!model.records){
                            ...
                            var param={flowPack:{},cardPacks:[],isQuick:false};
                            ...
                            this.targets&&this.targets.forEach(item=>{
                                param.flowPack={
                                    ...
                                    entity : JSON.stringify({
                                        reservationType:Sunset.Service.Dictionary.alias('RESERVATION_TYPE','CHECKUP'),
                            ...
                            model.checkType&&model.checkType.split(',').forEach(id=>{
                                param.cardPacks.push({
                                    viewName:'reservationcheckup',
                                    dataId:'',
                                    patientId:'',
                                    entity:JSON.stringify({
                                        hospitalCheckItemId:id.split('-')[0]
                                    }),
                            ...
                            return $http({
                                url: '/service/business/referralflow/checkup',
                                type: 'POST',
                                json:true,
                                data:param
                            }).then(res=>{
