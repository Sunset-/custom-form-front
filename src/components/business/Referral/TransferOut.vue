            ...
            <div class="flow_list_body">
                ...
                <div class="patient-flows-wrapper">
                    <sunset-datapage  v-ref:datapage :options="options" @refresh="refresh">
                        <referral-item v-for="item in flowList" :flow="item" :actived="currentItem.name" @check="checkFlow" @refreshlist="refreshList"></flow-item>
                    </sunset-datapage>
            ...
            return {
                ...
                options: {
                    ...
                    datasource: (filter) => {
                        ...
                        if(filter.status==Sunset.Service.Dictionary.alias('REFERRAL_OUT_STATUS','FINISHED')){
                            filter.extension= JSON.stringify({detailStatus:filter.status,isIncludePassiveClosed:true});
                        }else{
                ...
                leftFilterOps: {
                    fields: [{
                        ...
                        widget: 'radio',
                        type: 'button',
                        default: Sunset.Service.Dictionary.alias('REFERRAL_OUT_STATUS','PENDING'),
                        valuePlace: 'key',
                        textPlace: 'value',
                        changeFilter: true,
                        data() {
                            var enums = Sunset.Service.Dictionary.getEnums('REFERRAL_OUT_STATUS');
                            return enums;
                        }
                    }],
                    toolbar: {
                    ...
                    $http({
                        ...
                        res.forEach(item=>{
                            if(item.partyaCategory==Sunset.Service.Dictionary.alias('ORG_CODE','HOSPITAL')){
                                if(item.scenesType & Sunset.Service.Dictionary.alias('SCENES_CATEGORY','transfertreatment')){
            ...
            checkFlow(data){
                return $http({
                    ...
                    if(res&&res.hasPassiveClosed&&data[2]!='finish'){
                        ...
                        Router.go(`/Referral/TransferOut/${data[0]}/${data[1]}`);
                    }
                })
            }
        },
