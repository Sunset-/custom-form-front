...
<template>
    <div class="screening_patient_edit">
        <expert-selector v-ref:selector @submit="submitTeams"></expert-selector>
        <Modal
            :visible.sync="newRegistrationShow" title="提示" ok-text="去关闭" @on-ok="confirmFinish" @on-cancel="cancel">
        ...
        <div class="doctor_patient_content">
            ...
            <div v-show="current == null" class="card_page">
                <medicaldata-detail v-ref:detail @add-card="addCard" :patient-info="patientCardData" @signal="operateSignal"></medicaldata-detail>
            </div>
        </div>
        <!-- <div class="page_sidebar">
            <div class="system_btn">
                <div @click="back">返回</div>
                <div style="display:none"  @click="">消息</div>
                <div style="display:none"  @click="">收藏</div>
            </div>
            <div class="operate_btn">
                <div style="display:none"  @click="">进入视频</div>
                <div style="display:none" @click="">预约视频咨询</div>
                <div style="display:none" @click="chooseExpertBtn">提交专家</div>
            </div>
        </div> -->
    </div>
</template>
    ...
    export default {
            ...
            return {
                ...
                formOptions:{
                    ...
                    format:(model)=> {
                        ...
                        var param={};
                        param.patientId = this.$route.params.patientId;
                    ...
                    submit: (m) => {
                        return EyeScreenStore.newRegistration(m).then(res=>{
                    ...
                    if (item[2] == 'patientData') {
                        ...
                        if (item[3] && item[3] == 'PATIENT_TYPE') {
                            ...
                            if(visitingData[item[0]]){
                                var visitingDate = Sunset.Dates.format(new Date(visitingData[item[0]]),'yyyy-MM-dd');
        ...
        methods: {
            ...
            chooseExpert(){
                ...
                // para.type="";
                // para.keyword="";
                return EyeScreenStore.isTriaged(this.patientCardData.patientId).then(res=>{
            ...
            submitTeams(para){
                ...
                return EyeScreenStore.getDate().then(res=>{
                    if(res){
                        para.applyDate = res;
                        EyeScreenStore.saveExperts(para).then(res=>{
            ...
            newRegistration(){
                ...
                para.patientId = this.patientCardData.patientId;
                para.hospitalId = GlobalService.getCurrentUserSync().relation.hospital[0].id;
                return EyeScreenStore.isRegistrationExist(para).then(res=>{
                    if(res){
            ...
            confirmFinish () {
                ...
                Router.go(`/OUTPATIENT/DoctorOutpatient/remote/${data.id}`);
            },
            cancel () {
