...
<template>
	<div class="sunset-crud-table-container">
		...
		<div class="table-wrap sunset-crud-table-wrap">
			<sunset-loading :loading.sync="loading" top="40" :style="minWidth">
				...
				<div v-show="list.length==0" class="sunset-table-empty-tip" :style="options.emptyTipStyle">
					{{options.emptyTip||'无数据'}}
				</div>
			</sunset-loading>
		</div>
		<!--分页-->
			...
			recordToolsWidth() {
				if (this.options.recordToolsWidth) {
					...
					this.recordTools.forEach(t => {
						w += t.label.length * 14 + (t.icon ? 20 : 0) + 35;
					});
					w += 30;
					return w;
				}
