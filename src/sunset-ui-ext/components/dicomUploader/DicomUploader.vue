...
<script>
    ...
    export default {
        ...
        data() {
            return {
                uploadid: `sunset-widget-dicom-uploader-${++uid}`,
                uploadOptions: {
                    // swf文件路径
                    swf: '../../../vendor/webuploader/Uploader.swf',
                    // 文件接收服务端。
                    server: '/dicom-upload/api/1.0.0/file/wuploader',
                    // 选择文件的按钮。可选。
        ...
        methods: {
                    ...
                    Promise.resolve().then(() => {
                        ...
                        this.md5(files).then(md5s => {
                            return new Promise((resolve, reject) => {
                                $.ajax({
                                    url: "/dicom-upload/api/1.0.0/file/exist",
                                    dataType: "text",
                                    method: "post",
                                    data: {
                            ...
                            $http({
                                url: '/dicom-service/fileRes/uploads',
                                type: 'POST',
                                data: {
                                    userId: 'admin'
                                }
                                ...
                                if (exsitMd5s.length) {
                                    $.ajax({
                                        url: '/dicom-upload/api/1.0.0/file/fast',
                                        type: 'POST',
                                        data: {
                                            taskToken: taskToken,
                                            uploadType: 'dicom',
            ...
            startUpload() {
                GlobalService.uploadingStatus[this.uploadid] = true;
            ...
            finishUpload() {
                ...
                $.ajax({
                    url: `/dicom-upload/api/1.0.0/file/uploaded/${taskToken}/dicom`,
                    type: 'POST'
                }).then(res => {
            ...
            pollAnalysisResult: function (taskToken, time) {
                ...
                setTimeout(() => {
                    $http({
                        url: '/dicom-service/fileRes/resolved/',
                        type: 'GET',
                        data: {
                            taskToken: taskToken,
                            uploadType: 'dicom'
                        ...
                        if (res && res.flag) {
                            delete GlobalService.uploadingStatus[self.uploadid];
            ...
            mergeDicomInfo(studies) {
                studies.forEach(s => {
                    var study = this.studies.find(item => item.listId == s.listId);
                    if (study) {
                        study.imageCount = s.imageCount;
                    } else {
                        this.studies.push(s);
                    }
                });
